<%-- 
    ...
    Created on : Jun 13, 2025, 12:53:57 AM
    Author     : miran
--%>
...
<head>
    ...
    <title>Patel Hardware - All Items</title>
    <!-- Tailwind CSS CDN -->
...
<body class="min-h-screen flex flex-col">
    ...
    <main class="flex-grow container mx-auto p-4 md:p-8">
        <h2 class="text-4xl font-extrabold text-gray-900 text-center mb-8">All Available Items</h2>

        <%-- Display messages (success/error) from BuyItemsServlet --%>
        <% 
        ...
        <c:choose>
                    ...
                    <c:forEach var="item" items="${itemList}">
                        <div class="item-card p-6 flex flex-col justify-between">
                            ...
                            <div>
                                ...
                                <p class="text-sm ${item.available ? 'available-status' : 'unavailable-status'} mb-4">
                                    Status: ${item.available ? 'Available' : 'Out of Stock'}
                                </p>
                                <c:if test="${item.available}">
                                    <form action="buy-item" method="POST">
                                        <input type="hidden" name="itemId" value="${item.itemId}">
                                        <%-- Optional: allow user to input quantity --%>
                                        <div class="flex items-center justify-between mb-4">
                                            <label for="quantity-${item.itemId}" class="text-gray-700 text-sm mr-2">Quantity:</label>
                                            <input type="number" id="quantity-${item.itemId}" name="quantity" value="1" min="1" class="w-20 px-2 py-1 border rounded-md text-center text-sm">
                                        </div>
                                        <button type="submit" class="btn-buy w-full">Add to Cart</button>
                                    </form>
                                </c:if>
                                <c:if test="${!item.available}">
                                    <button class="btn-buy w-full opacity-50 cursor-not-allowed" disabled>Out of Stock</button>
                                </c:if>
                            </div>
                        </div>
                    </c:forEach>
            ...
            <c:otherwise>
                <div class="text-center text-gray-600 text-xl py-10">
                    <p>No items found or available at this time.</p>
